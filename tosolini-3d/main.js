...
function transitionTo(index, fast = false, fromIndex = activeIndex) {
  ...
  const stops = [{ position: camera.position.clone(), quaternion: camera.quaternion.clone() }];
  const step = Math.sign(index - fromIndex);
  if (fast && step) {
    for (let i = fromIndex + step; step > 0 ? i <= index : i >= index; i += step) stops.push(poses[i]);
  } else stops.push(poses[index]);
  transition = { stops, start: performance.now(), leg: fast ? 380 : 1050 };
}
...
function setActive(index, fast = false) {
  if (index === activeIndex && !fast) return;
  ...
  transitionTo(index, fast, previous);
}
